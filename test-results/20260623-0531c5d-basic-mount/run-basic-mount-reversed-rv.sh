...
img="$WORK/reversed-rv.img"
cp "$ROOT/disk.img" "$img"
cat > "$WORK/contest-basic-mount-reversed.sh" <<'EOF'
#!/bin/sh
...
mcopy -o -i "$img" "$WORK/contest-basic-mount-reversed.sh" ::/contest.sh
...
timeout --foreground 180s qemu-system-riscv64 \
    ...
    -netdev user,id=net,hostfwd=tcp::12056-:80 \
    -device virtio-net-device,netdev=net,bus=virtio-mmio-bus.4 \
    -drive "file=$img,if=none,format=raw,id=x1" \
    -device virtio-blk-device,drive=x1,bus=virtio-mmio-bus.1 \
    -no-reboot > "$OUT/after-rv-basic-mount-reversed.txt" 2>&1
rc=$?
set -e
echo "$rc" > "$OUT/after-rv-basic-mount-reversed.status"
